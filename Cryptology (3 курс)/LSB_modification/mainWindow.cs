...
{ 
    public partial class mainWindow : Form
    {
        ...
        public void decodeImage()
        {
            ...
            {
                DES des = new DES(temp, maskedTextBox1.Text, true);
                temp = DES.Begin();
            }
            else
                MessageBox.Show("Пароль не был задан. Обычный режим LSB");
            messageShow.Text = temp;// показать результат (немного отредактирован для читабельности)
        ...
        private void Coder_Click(object sender, EventArgs e)
        {
            if (Checking())
            {
                if (!String.IsNullOrEmpty(maskedTextBox1.Text))
                {
                    DES des = new DES(message, maskedTextBox1.Text, false);
                    message = DES.Begin();
                }
                else
                    MessageBox.Show("Пароль не был задан. Обычный режим LSB");
